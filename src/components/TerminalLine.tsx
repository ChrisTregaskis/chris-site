  ...
  return (
    <div className="relative group">
      <div className="flex gap-2">
        <p className="text-light text-1xl min-w-max">{yourcomputer}</p>
        {skipAnimation ? (
          <>
            <p className="text-1xl">
              {typedWords.map((word, i) => (
                <span key={i} className={cn("", word.className)}>
                  {word.text}
                </span>
              ))}
            </p>
          ...
          <Typewriter words={typedWords} animationSpeed={animationSpeed} />
        )}
